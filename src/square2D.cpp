...
square2D::square2D(vec2f center, float radius) : m_center(center), m_radius(radius)
{
    ...
    m_corners.push_back(flipX(m_center, top_right)); // Top left
    m_corners.push_back(flip(m_center, top_right)); // Bottom left
    m_corners.push_back(flipY(m_center, top_right));  // Bottom right
}
